...
cd "$ROOT"
OUT_ROOT="autoresearch/loop-260718-0223/checkpoint_round5"
...
base="runs/planner_aligned_q_rank_v5_context_safe"
variants=(best best_h1 best_h2 best_h3 best_h4 best_h5 epoch10 epoch12)
checkpoints=(
    "$base/best.pt" "$base/best_h1.pt" "$base/best_h2.pt" "$base/best_h3.pt"
    "$base/best_h4.pt" "$base/best_h5.pt" "$base/epoch_010.pt" "$base/epoch_012.pt"
)
...
run_one() {
    local variant="$1" checkpoint="$2" circuit="$3" benchmark="$4" gpu="$5"
    local out="$OUT_ROOT/$variant/$circuit"
    local log="$OUT_ROOT/logs/${variant}__${circuit}.log"
    echo "[round5] start variant=$variant circuit=$circuit gpu=$gpu checkpoint=$checkpoint"
    CUDA_VISIBLE_DEVICES="$gpu" python scripts/run_gmean_sweep.py \
        ...
        | sed -u "s|^|[round5/$variant/$circuit] |" | tee "$log"
    echo "[round5] done variant=$variant circuit=$circuit gpu=$gpu"
}
...
for variant_index in "${!variants[@]}"; do
    for circuit_index in "${!circuits[@]}"; do
        gpu="${gpus[$((job % ${#gpus[@]}))]}"
        run_one "${variants[$variant_index]}" "${checkpoints[$variant_index]}" \
...
for index in "${!pids[@]}"; do
    if ! wait "${pids[$index]}"; then
        echo "[round5] failed job=${labels[$index]}"
        failed=1
    fi
done
