...
cd "$ROOT"
OUT_ROOT="autoresearch/loop-260718-0223/round13_winner_five"
...
run_one() {
    ...
    echo "[round13-five] start circuit=$circuit gpu=$gpu"
    CUDA_VISIBLE_DEVICES="$gpu" python scripts/run_gmean_sweep.py \
        ...
        | sed -u "s|^|[round13-five/$circuit] |" | tee "$OUT_ROOT/logs/$circuit.log"
    echo "[round13-five] done circuit=$circuit gpu=$gpu"
}
