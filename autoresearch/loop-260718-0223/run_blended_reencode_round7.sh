...
cd "$ROOT"
OUT_ROOT="autoresearch/loop-260718-0223/blended_reencode_round7"
mkdir -p "$OUT_ROOT/logs"
export TPI_HARD_CLUSTER_MAX_HARD_NODES=1024 TPI_LATENT_NORM_CLIP_RATIO=0
export TPI_LATENT_REENCODE_INTERVAL=32 TPI_PLAN_THREADS=1 OMP_NUM_THREADS=1 MKL_NUM_THREADS=1

variants=(blend005 blend010 blend020 blend040)
blends=(0.05 0.10 0.20 0.40)
circuits=(b22_C b17_C)
benchmarks=(iscas99__b22 iscas99__b17)
gpus=(1 4 5 6)

run_one() {
    local variant="$1" blend="$2" circuit="$3" benchmark="$4" gpu="$5"
    local out="$OUT_ROOT/$variant/$circuit"
    local log="$OUT_ROOT/logs/${variant}__${circuit}.log"
    echo "[round7] start variant=$variant circuit=$circuit gpu=$gpu blend=$blend"
    TPI_LATENT_REENCODE_BLEND="$blend" CUDA_VISIBLE_DEVICES="$gpu" \
    python scripts/run_gmean_sweep.py \
        ...
        | sed -u "s|^|[round7/$variant/$circuit] |" | tee "$log"
    echo "[round7] done variant=$variant circuit=$circuit gpu=$gpu"
}
...
for variant_index in "${!variants[@]}"; do
    for circuit_index in "${!circuits[@]}"; do
        gpu="${gpus[$((job % ${#gpus[@]}))]}"
        run_one "${variants[$variant_index]}" "${blends[$variant_index]}" \
...
for index in "${!pids[@]}"; do
    if ! wait "${pids[$index]}"; then
        echo "[round7] failed job=${labels[$index]}"
        failed=1
    fi
done
