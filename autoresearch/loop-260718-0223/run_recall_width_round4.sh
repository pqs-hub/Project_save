...
cd "$ROOT"
OUT_ROOT="autoresearch/loop-260718-0223/recall_width_round4"
...
variants=(union_c48 union_c64 union_c80 union_r96_m48)
counts=(48 64 80 96)
models=("" "" "" 48)
...
run_one() {
    local variant="$1" count="$2" model_count="$3" circuit="$4" benchmark="$5" gpu="$6"
    local out="$OUT_ROOT/$variant/$circuit"
    local log="$OUT_ROOT/logs/${variant}__${circuit}.log"
    local staged=()
    if [[ -n "$model_count" ]]; then staged=(--k-models "$model_count"); fi
    echo "[round4] start variant=$variant circuit=$circuit gpu=$gpu candidates=$count model=$model_count"
    CUDA_VISIBLE_DEVICES="$gpu" python scripts/run_gmean_sweep.py \
        ...
        --beam-widths 1 --lookahead-depths 1 --max-candidates "$count" "${staged[@]}" \
        --discount-gammas 0.9 --candidate-strategies hard_fault_recall_union \
        ...
        | sed -u "s|^|[round4/$variant/$circuit] |" | tee "$log"
    echo "[round4] done variant=$variant circuit=$circuit gpu=$gpu"
}
...
for variant_index in "${!variants[@]}"; do
    for circuit_index in "${!circuits[@]}"; do
        gpu="${gpus[$((job % ${#gpus[@]}))]}"
        run_one "${variants[$variant_index]}" "${counts[$variant_index]}" \
            "${models[$variant_index]}" "${circuits[$circuit_index]}" \
...
for index in "${!pids[@]}"; do
    if ! wait "${pids[$index]}"; then
        echo "[round4] failed job=${labels[$index]}"
        failed=1
    fi
done
