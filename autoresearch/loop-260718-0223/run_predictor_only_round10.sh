...
cd "$ROOT"
OUT_ROOT="autoresearch/loop-260718-0223/predictor_only_round10"
...
variants=(dyn_strong dyn_weak dyn_jepa)
q_ranks=(0.38 0.10 0.0)
q_values=(0.08 0.02 0.0)
candidate_weights=(0.18 0.05 0.0)
context_weights=(0.25 0.05 0.0)
gpus=(0 1 2)
...
build_config() {
    ...
    jq \
        --arg labels "$LABELS" --arg run_dir "$OUT_ROOT/runs/$variant" --arg init "$INIT" \
        --argjson q_rank "${q_ranks[$index]}" --argjson q_value "${q_values[$index]}" \
        --argjson candidate_weight "${candidate_weights[$index]}" \
        --argjson context_weight "${context_weights[$index]}" \
        '.labels=$labels | .run_dir=$run_dir | .init_checkpoint=$init | .init_checkpoint_strict=true
         | .trainable_modules="rollout_dynamics" | .epochs=4 | .lr=0.00005
         | .max_train_samples=8000 | .max_train_steps_per_epoch=120
         | .max_val_samples=2048 | .max_val_steps=32
         | .lambda_jepa=1.0 | .lambda_scoap=0.0 | .lambda_delta_scoap=0.0
         | .lambda_hard=0.0 | .lambda_hard_rank=0.0 | .lambda_hard_brier=0.0
         | .lambda_hard_soft_f1=0.0 | .lambda_hard_count=0.0 | .lambda_hard_reduction=0.0
         | .lambda_fc=0.0 | .lambda_pattern=0.0 | .lambda_return=0.0
         | .lambda_q_rank=$q_rank | .lambda_q_value=$q_value
         | .lambda_candidate=$candidate_weight | .lambda_context_rank=$context_weight
         | .lambda_ndcg_rank=0.0 | .lambda_conservative_q=0.0
         | .rollout_training=true | .rollout_max_horizon=32
         | .rollout_start_epoch=1 | .rollout_increase_every=1
         | .rollout_start_horizon=8 | .rollout_horizon_increment=8
         | .require_full_horizon=true | .repeat_train_samples=false | .device="cuda"' \
        configs/planner_aligned_q_rank_v5_context_safe.json > "$OUT_ROOT/configs/$variant.json"
}

train_one() {
    local index="$1" variant="${variants[$1]}" gpu="${gpus[$1]}"
    echo "[round10] train_start variant=$variant gpu=$gpu"
    CUDA_VISIBLE_DEVICES="$gpu" python -m tpi_jepa.train --config "$OUT_ROOT/configs/$variant.json" 2>&1 \
        | sed -u "s|^|[round10/train/$variant] |" | tee "$OUT_ROOT/logs/$variant.train.log"
    echo "[round10] train_done variant=$variant gpu=$gpu"
}

eval_one() {
    local variant="$1" score="$2" gpu="$3" tag="${variant}__${score}"
    echo "[round10] b15_start variant=$variant score=$score gpu=$gpu"
    ...
    CUDA_VISIBLE_DEVICES="$gpu" python scripts/run_gmean_sweep.py \
        ...
        | sed -u "s|^|[round10/b15/$tag] |" | tee "$OUT_ROOT/logs/$tag.b15.log"
    echo "[round10] b15_done variant=$variant score=$score gpu=$gpu"
}
