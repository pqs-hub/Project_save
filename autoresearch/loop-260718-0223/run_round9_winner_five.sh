...
cd "$ROOT"
OUT_ROOT="autoresearch/loop-260718-0223/round9_winner_five"
CHECKPOINT="autoresearch/loop-260718-0223/structural_rollout_round9/runs/h32_strong/best_final_horizon.pt"
...
run_one() {
    ...
    local out="$OUT_ROOT/$circuit"
    local log="$OUT_ROOT/logs/$circuit.log"
    echo "[round9-five] start circuit=$circuit gpu=$gpu"
    CUDA_VISIBLE_DEVICES="$gpu" python scripts/run_gmean_sweep.py \
        ...
        --planners greedy --score-fields guarded_reward_context --beam-objectives cumulative \
        ...
        | sed -u "s|^|[round9-five/$circuit] |" | tee "$log"
    echo "[round9-five] done circuit=$circuit gpu=$gpu"
}
