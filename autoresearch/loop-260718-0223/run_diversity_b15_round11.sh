...
cd "$ROOT"
OUT_ROOT="autoresearch/loop-260718-0223/diversity_b15_round11"
...
run_one() {
    ...
    echo "[round11] start variant=$variant gpu=$gpu depth=$depth penalty=$penalty"
    CUDA_VISIBLE_DEVICES="$gpu" python scripts/run_gmean_sweep.py \
        ...
        | sed -u "s|^|[round11/$variant] |" | tee "$log"
    echo "[round11] done variant=$variant gpu=$gpu"
}
...
for index in "${!pids[@]}"; do
    if ! wait "${pids[$index]}"; then
        echo "[round11] failed variant=${labels[$index]}"
        failed=1
    fi
done
