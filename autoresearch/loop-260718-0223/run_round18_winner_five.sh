...
cd "$ROOT"
OUT_ROOT="autoresearch/loop-260718-0223/round18_winner_five"
...
export TPI_ADAPTIVE_EXPANSION_MARGIN=0.001
...
run_one() {
    ...
    echo "[round18-five] start circuit=$circuit gpu=$gpu"
    CUDA_VISIBLE_DEVICES="$gpu" python scripts/run_gmean_sweep.py \
        ...
        | sed -u "s|^|[round18-five/$circuit] |" | tee "$OUT_ROOT/logs/$circuit.log"
    echo "[round18-five] done circuit=$circuit gpu=$gpu"
}
...
root = Path("autoresearch/loop-260718-0223/round18_winner_five")
payload = {}
for circuit in ("b15_C", "b20_C", "b21_C", "b22_C", "b17_C"):
    path = glob.glob(str(root / circuit / "plans" / "*.csv"))[0]
    rows = list(csv.DictReader(open(path)))
    expanded = sum(row.get("adaptive_expanded", "").lower() == "true" for row in rows)
    payload[circuit] = {"steps": len(rows), "expanded": expanded, "expanded_pct": 100.0 * expanded / len(rows)}
print(json.dumps(payload, indent=2))
(root / "adaptive_expansion_audit.json").write_text(json.dumps(payload, indent=2) + "\n")
PY
