...
cd "$ROOT"
gpu="${GPU:-7}"
out="${OUT_DIR:-autoresearch/improve-260716-1344/round16_b17/context_cluster_c32_seed1024_depth4}"
mkdir -p "$(dirname "$out")/console_logs"

echo "[round16] start tag=context_cluster_c32_seed1024_depth4 gpu=$gpu"
CUDA_VISIBLE_DEVICES="$gpu" python scripts/run_gmean_sweep.py \
  ...
  --beam-widths 1 --lookahead-depths 1 --max-candidates 32 \
  ...
  --stream-logs 2>&1 | tee "$(dirname "$out")/console_logs/context_cluster_c32_seed1024_depth4.log"
echo "[round16] done tag=context_cluster_c32_seed1024_depth4 gpu=$gpu"
