...
cd "$ROOT"
gpu="${GPU:-7}"
out="${OUT_DIR:-autoresearch/improve-260716-1344/round5_b17/context_cluster_c48_seed1024_prior}"
mkdir -p "$(dirname "$out")/console_logs"

echo "[round5] start tag=context_cluster_c48_seed1024_prior gpu=$gpu"
CUDA_VISIBLE_DEVICES="$gpu" python scripts/run_gmean_sweep.py \
  ...
  --stream-logs 2>&1 | tee "$(dirname "$out")/console_logs/context_cluster_c48_seed1024_prior.log"
echo "[round5] done tag=context_cluster_c48_seed1024_prior gpu=$gpu"
