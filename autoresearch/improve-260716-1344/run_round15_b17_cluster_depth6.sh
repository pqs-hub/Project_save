...
cd "$ROOT"
gpu="${GPU:-7}"
out="${OUT_DIR:-autoresearch/improve-260716-1344/round15_b17/context_cluster_c48_seed1024_depth6}"
mkdir -p "$(dirname "$out")/console_logs"

echo "[round15] start tag=context_cluster_c48_seed1024_depth6 gpu=$gpu"
CUDA_VISIBLE_DEVICES="$gpu" python scripts/run_gmean_sweep.py \
  ...
  --stream-logs 2>&1 | tee "$(dirname "$out")/console_logs/context_cluster_c48_seed1024_depth6.log"
echo "[round15] done tag=context_cluster_c48_seed1024_depth6 gpu=$gpu"
