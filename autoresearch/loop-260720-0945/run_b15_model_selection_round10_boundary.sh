...
jobs=(
  return_within_lr2e4:003 return_within_lr2e4:005 return_within_lr2e4:007
  return_within_lr5e4:003 return_within_lr5e4:005 return_within_lr5e4:007
  return_dual_lr2e4:003 return_dual_lr2e4:005 return_dual_lr2e4:007
  return_dual_lr5e5:007
)
...
run_one() {
  ...
  echo "[b15-r10-boundary] start variant=$variant epoch=$epoch gpu=$gpu"
  CUDA_VISIBLE_DEVICES="$gpu" python -u scripts/run_gmean_sweep.py \
    ...
    2>&1 | sed -u "s|^|[b15-r10-boundary/$variant/e$epoch] |" \
    | tee "$OUT/logs/${variant}__e${epoch}.log"
}

pids=(); failed=0; job_index=0
wait_batch() {
  local pid
