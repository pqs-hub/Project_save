...
mkdir -p "$BASE/logs"
python scripts/prepare_cone_round6.py
...
run_one() {
  ...
  echo "[train-r6] start variant=$variant gpu=$gpu"
  CUDA_VISIBLE_DEVICES="$gpu" python -u -m tpi_jepa.train --config "$BASE/configs/$variant.json" \
    2>&1 | sed -u "s|^|[train-r6/$variant] |" | tee "$BASE/logs/$variant.log"
}
...
echo "[train-r6] complete variants=${variants[*]}"
