...
BASE=$LOOP/model_training_round20_round10_direct_return_b15
...
INCUMBENT=$LOOP/model_training_round19_direct_return_width_b15/b15_selection.json
TAGS=(within_lr5e5 within_lr1e4 within_lr2e4 dual_lr2e4)
VARIANTS=(return_within_lr5e5 return_within_lr1e4 return_within_lr2e4 return_dual_lr2e4)
GPU=${GPU:-0}
...
for index in "${!TAGS[@]}"; do
  ...
  checkpoint=$LOOP/model_training_round10/runs/$variant/best_final_horizon.pt
  echo "[r20-r10-return-plan] start tag=$tag gpu=$GPU"
  CUDA_VISIBLE_DEVICES="$GPU" python -u -m tpi_jepa.plan \
    ...
    2>&1 | sed -u "s|^|[r20-r10-return-plan/$tag] |" | tee "$BASE/logs/plan_${tag}.log" &
  pids+=("$!")
done
wait_all "${pids[@]}"
...
pids=()
for tag in "${TAGS[@]}"; do
  echo "[r20-r10-return-eval] start tag=$tag"
  python -u -m tpi_jepa.evaluate_plan_tmax \
    ...
    2>&1 | sed -u "s|^|[r20-r10-return-eval/$tag] |" | tee "$BASE/logs/eval_${tag}.log" &
  pids+=("$!")
done
wait_all "${pids[@]}"
...
python scripts/select_b15_tail_variants.py \
  ...
  --variants within_lr5e5,within_lr1e4,within_lr2e4,dual_lr2e4 \
  --prefer-order within_lr1e4,within_lr5e5,within_lr2e4,dual_lr2e4 \
