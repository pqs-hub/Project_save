...
run_one() {
  local variant=$1 alpha=$2 decay=$3 gpu=$4
  local alpha_tag=${alpha/./p}
  local decay_tag=$decay
  if [[ "$decay" == 0 ]]; then decay_tag=none; fi
  local checkpoint="$BASE/runs/$variant/best_final_horizon.pt"
  local out="$OUT/$variant/best_final_horizon/alpha_${alpha_tag}_decay_${decay_tag}"
  echo "[b15-r9] start variant=$variant alpha=$alpha decay=$decay gpu=$gpu"
  ...
  CUDA_VISIBLE_DEVICES="$gpu" python -u scripts/run_gmean_sweep.py \
    ...
    2>&1 | sed -u "s|^|[b15-r9/$variant/a$alpha/d$decay] |" \
    | tee "$OUT/logs/${variant}__a${alpha}__d${decay}.log"
}
...
for variant in "${variants[@]}"; do
  for alpha in "${alphas[@]}"; do
    for decay in "${decays[@]}"; do
      gpu=${gpus[$((job % ${#gpus[@]}))]}
      run_one "$variant" "$alpha" "$decay" "$gpu" &
      pids+=("$!")
      job=$((job+1))
