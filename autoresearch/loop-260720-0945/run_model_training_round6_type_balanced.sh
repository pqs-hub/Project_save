...
mkdir -p "$BASE/logs"
python scripts/prepare_type_balanced_round6.py
...
run_one() {
  ...
  echo "[train-r6-type] start variant=$variant gpu=$gpu"
  CUDA_VISIBLE_DEVICES="$gpu" python -u -m tpi_jepa.train --config "$BASE/configs/$variant.json" \
    2>&1 | sed -u "s|^|[train-r6-type/$variant] |" | tee "$BASE/logs/$variant.log"
}
...
echo "[train-r6-type] complete variants=${variants[*]}"
