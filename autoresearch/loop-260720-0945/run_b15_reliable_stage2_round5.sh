...
OUT="$BASE/b15_selection/reliable_stage2"
mkdir -p "$OUT/logs"
...
variants=(prefix_cql_sa prefix_toplist_sa)
checkpoints=(best_final_horizon epoch_012)
# alpha, minimum reliable heads for OP/CP1; CP0 always requires both.
settings=(
  "0.10 1"
  "0.20 1"
  "0.40 1"
...
IFS=',' read -r -a gpus <<< "${GPUS_CSV:-0,0}"
...
run_one() {
  local variant=$1 checkpoint_tag=$2 alpha=$3 min_heads=$4 gpu=$5
  local checkpoint="$BASE/runs/$variant/$checkpoint_tag.pt"
  local tag="a${alpha/./p}_h${min_heads}_c2"
  local out="$OUT/$variant/$checkpoint_tag/$tag"
  echo "[b15-reliable] start variant=$variant checkpoint=$checkpoint_tag alpha=$alpha heads=$min_heads gpu=$gpu"
  TPI_TYPED_RESIDUAL_ALPHA="$alpha" TPI_TYPED_RESIDUAL_DECAY_STEPS=16 \
  TPI_TYPED_RELIABLE_MIN_HEADS="$min_heads" TPI_TYPED_RELIABLE_CP0_MIN_HEADS=2 \
  ...
  CUDA_VISIBLE_DEVICES="$gpu" \
    python -u scripts/run_gmean_sweep.py \
      ...
      2>&1 | sed -u "s|^|[b15-reliable/$variant/$checkpoint_tag/$tag] |" \
...
for variant in "${variants[@]}"; do
  for checkpoint_tag in "${checkpoints[@]}"; do
    for setting in "${settings[@]}"; do
      read -r alpha min_heads <<< "$setting"
      gpu=${gpus[$((job % ${#gpus[@]}))]}
      run_one "$variant" "$checkpoint_tag" "$alpha" "$min_heads" "$gpu" &
      pids+=("$!")
      job=$((job+1))
...
echo "[b15-reliable] complete jobs=$job"
