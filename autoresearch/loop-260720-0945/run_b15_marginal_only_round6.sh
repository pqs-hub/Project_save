...
OUT="$BASE/b15_selection/marginal_only"
mkdir -p "$OUT/logs"
...
export TPI_TYPED_RESIDUAL_CLIP=1.0 TPI_TYPED_RESIDUAL_DISAGREEMENT_BETA=0.0
# The fixed-prefix diagnostic identifies the cone marginal head as the only
# consistently high-rank head.  Non-CP0 actions therefore use that residual;
# CP0 still needs support from both marginal and return heads.
export TPI_TYPED_RELIABLE_MARGINAL_WEIGHT=1.0
...
variants=(cone_rank cone_toplist cone_toplist_sa)
...
run_one() {
  local variant=$1 checkpoint_tag=$2 alpha=$3 gpu=$4
  local alpha_tag=${alpha/./p}
  local out="$OUT/$variant/$checkpoint_tag/alpha_${alpha_tag}_decay_16"
  local checkpoint="$BASE/runs/$variant/$checkpoint_tag.pt"
  echo "[b15-r6-marginal] start variant=$variant checkpoint=$checkpoint_tag alpha=$alpha gpu=$gpu"
  ...
  CUDA_VISIBLE_DEVICES="$gpu" \
    python -u scripts/run_gmean_sweep.py \
      ...
      2>&1 | sed -u "s|^|[b15-r6-marginal/$variant/$checkpoint_tag/a$alpha] |" \
      | tee "$OUT/logs/${variant}__${checkpoint_tag}__a${alpha}.log"
}
...
for variant in "${variants[@]}"; do
  for checkpoint_tag in "${checkpoints[@]}"; do
    for alpha in "${alphas[@]}"; do
      gpu=${gpus[$((job % ${#gpus[@]}))]}
      run_one "$variant" "$checkpoint_tag" "$alpha" "$gpu" &
      pids+=("$!")
      job=$((job+1))
...
python scripts/select_onpolicy_typed_on_b15.py --base "$BASE" \
  --incumbent-manifest autoresearch/loop-260720-0945/model_training_round5/b15_selection/winner.json
