...
OUT=$LOOP/typed_winner_round20_round10_epoch8_five
INCUMBENT=$LOOP/typed_winner_round8_five
SELECTION=$LOOP/model_training_round20_round10_epoch8_direct_return_b15
CHECKPOINT=$LOOP/model_training_round10/runs/return_within_lr5e4/epoch_008.pt
...
# b15 is the sole model/checkpoint/config selection circuit.  Reuse its
# completed selection artifact and freeze that checkpoint for all validation
# circuits below.
cp "$SELECTION/plans/within_lr5e4_epoch8.csv" \
  "$OUT/b15_C/plans/iscas99__b15_1.csv"
cp -a "$SELECTION/evals/within_lr5e4_epoch8/." \
...
for index in 1 2 3 4; do
  ...
  echo "[r20-five-plan] start circuit=$circuit gpu=$gpu budget=$budget refresh=$refresh_steps score=typed_return_pred"
  CUDA_VISIBLE_DEVICES="$gpu" /usr/bin/time -f '%e' -o "$OUT/$circuit/timing/plan_sec.txt" \
    python -u -m tpi_jepa.plan \
      ...
      2>&1 | sed -u "s|^|[r20-five-plan/$circuit] |" | tee "$OUT/$circuit/logs/plan.log" &
  pids+=("$!")
done
wait_all "${pids[@]}"
...
for index in 1 2 3 4; do
  circuit=${circuits[$index]}
  benchmark=${benchmarks[$index]}
  echo "[r20-five-eval] start circuit=$circuit"
  /usr/bin/time -f '%e' -o "$OUT/$circuit/timing/final_eval_sec.txt" \
    python -u -m tpi_jepa.evaluate_plan_tmax \
      ...
      2>&1 | sed -u "s|^|[r20-five-eval/$circuit] |" | tee "$OUT/$circuit/logs/final_eval.log" &
  pids+=("$!")
done
wait_all "${pids[@]}"
