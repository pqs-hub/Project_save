...
OUT=$LOOP/typed_winner_round14_candidate_prior_five
...
export TPI_CANDIDATE_PRIOR_ALPHA
...
# b15 is the selection run itself; reuse its already evaluated alpha=0.01 artifact.
cp "$LOOP/model_training_round14_candidate_prior_b15/plans/alpha_0p010.csv" \
  "$OUT/b15_C/plans/iscas99__b15_1.csv"
cp -a "$LOOP/model_training_round14_candidate_prior_b15/evals/alpha_0p010/." \
...
for index in 1 2 3 4; do
  ...
  echo "[r14-five-plan] start circuit=$circuit gpu=$gpu budget=$budget refresh=$refresh_steps alpha=$CANDIDATE_PRIOR_ALPHA"
  CUDA_VISIBLE_DEVICES="$gpu" /usr/bin/time -f '%e' -o "$OUT/$circuit/timing/plan_sec.txt" \
    python -u -m tpi_jepa.plan \
      ...
      2>&1 | sed -u "s|^|[r14-five-plan/$circuit] |" | tee "$OUT/$circuit/logs/plan.log" &
  pids+=("$!")
done
wait_all "${pids[@]}"
...
for index in 1 2 3 4; do
  circuit=${circuits[$index]}
  benchmark=${benchmarks[$index]}
  echo "[r14-five-eval] start circuit=$circuit"
  /usr/bin/time -f '%e' -o "$OUT/$circuit/timing/final_eval_sec.txt" \
    python -u -m tpi_jepa.evaluate_plan_tmax \
      ...
      2>&1 | sed -u "s|^|[r14-five-eval/$circuit] |" | tee "$OUT/$circuit/logs/final_eval.log" &
  pids+=("$!")
done
wait_all "${pids[@]}"
...
python scripts/materialize_residual_refresh_five.py \
  ...
  --candidate-prior-alpha "$CANDIDATE_PRIOR_ALPHA" \
