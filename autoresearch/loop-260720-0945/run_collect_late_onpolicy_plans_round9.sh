...
python -u scripts/prepare_late_horizon_round9.py | tee "$ROOT/prepare_late_sources.log"
...
run_plan() {
  ...
  if [[ -s "$out" ]] && (( $(wc -l < "$out") >= 257 )); then
    echo "[onpolicy-r9] skip complete benchmark=$benchmark out=$out"
    return 0
  fi
  echo "[onpolicy-r9] start benchmark=$benchmark gpu=$gpu"
  CUDA_VISIBLE_DEVICES="$gpu" python -u -m tpi_jepa.plan \
    ...
    2>&1 | sed -u "s|^|[onpolicy-r9/$benchmark] |" | tee "$PLANS/logs/$benchmark.log"
  echo "[onpolicy-r9] done benchmark=$benchmark gpu=$gpu"
}
...
echo "[onpolicy-r9] plans_complete count=${#benchmarks[@]} out=$PLANS"
