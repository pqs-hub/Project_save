...
OUT="$BASE/b15_selection/cone_local"
CHECKPOINT="$BASE/runs/cone_toplist/best_final_horizon.pt"
mkdir -p "$OUT/logs"
...
# Local refinement around the b15-only Round6 leader (alpha=.20, decay=16).
# The already evaluated center point is omitted.
settings=(
  "0.15 24" "0.15 32"
  "0.20 8"  "0.20 24" "0.20 32" "0.20 48"
  "0.25 8"  "0.25 12" "0.25 16" "0.25 24" "0.25 32"
  "0.30 8"  "0.30 12" "0.30 16" "0.30 24"
  "0.40 8"  "0.40 12" "0.40 16"
  "0.05 0"  "0.10 0"
)
...
run_one() {
  ...
  local out="$OUT/alpha_${alpha_tag}_decay_$decay"
  echo "[b15-r6-local] start alpha=$alpha decay=$decay gpu=$gpu"
  ...
  CUDA_VISIBLE_DEVICES="$gpu" \
    python -u scripts/run_gmean_sweep.py \
      ...
      2>&1 | sed -u "s|^|[b15-r6-local/a$alpha/d$decay] |" \
      | tee "$OUT/logs/a${alpha}__d${decay}.log"
}
