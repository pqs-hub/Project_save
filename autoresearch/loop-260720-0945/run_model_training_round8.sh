...
mkdir -p "$BASE/logs"
python scripts/prepare_moe_round8.py
variants=(moe_experts_within moe_joint_within)
IFS=',' read -r -a gpus <<< "${GPUS_CSV:-6,7}"
...
run_one() {
  ...
  echo "[train-r8] start variant=$variant gpu=$gpu"
  CUDA_VISIBLE_DEVICES="$gpu" python -u -m tpi_jepa.train --config "$BASE/configs/$variant.json" \
    2>&1 | sed -u "s|^|[train-r8/$variant] |" | tee "$BASE/logs/$variant.log"
}
...
echo "[train-r8] complete variants=${variants[*]}"
