...
variants=(type_balanced_rank type_balanced_toplist)
checkpoints=(best_final_horizon epoch_008)
# score alpha marginal_weight disagreement
settings=(
  "q_typed_residual_context 0.05 0.75 0.25"
  "q_typed_residual_context 0.10 0.75 0.25"
  "q_typed_residual_context 0.20 0.75 0.25"
  "q_typed_reliable_context 0.10 1.00 0.00"
  "q_typed_reliable_context 0.20 1.00 0.00"
)
...
run_one() {
  local variant=$1 checkpoint_tag=$2 score=$3 alpha=$4 weight=$5 disagreement=$6 gpu=$7
  ...
  echo "[b15-r6-type] start variant=$variant checkpoint=$checkpoint_tag score=$score alpha=$alpha weight=$weight gpu=$gpu"
  ...
  CUDA_VISIBLE_DEVICES="$gpu" \
    python -u scripts/run_gmean_sweep.py \
      ...
      2>&1 | sed -u "s|^|[b15-r6-type/$variant/$checkpoint_tag/$score/a$alpha/w$weight] |" \
...
for variant in "${variants[@]}"; do
  for checkpoint_tag in "${checkpoints[@]}"; do
    for setting in "${settings[@]}"; do
      read -r score alpha weight disagreement <<< "$setting"
      gpu=${gpus[$((job % ${#gpus[@]}))]}
      run_one "$variant" "$checkpoint_tag" "$score" "$alpha" "$weight" "$disagreement" "$gpu" &
      pids+=("$!")
      job=$((job+1))
