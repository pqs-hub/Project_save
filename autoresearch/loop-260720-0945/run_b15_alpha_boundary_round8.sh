...
mapfile -t selected < <(python -c \
  'import json,sys; w=json.load(open(sys.argv[1]))["winner"]; e=w.get("planner_environment") or {}; print(w["checkpoint"]); print(w["score_field"]); print(w["typed_residual_decay_steps"]); print(w.get("typed_reliable_marginal_weight",0.75)); print(w.get("typed_reliable_min_heads",1)); print(w.get("typed_reliable_cp0_min_heads",2)); print(w.get("adaptive_base_candidates",48)); print(w.get("adaptive_expansion_margin",0.003)); print(w.get("adaptive_margin_mode","relative_range")); print(w.get("max_candidates",64)); print(w.get("candidate_strategy","hard_fault_cluster")); print(e.get("TPI_LATENT_REENCODE_INTERVAL",0)); print(e.get("TPI_LATENT_REENCODE_BLEND",1))' \
  "$WINNER")
CHECKPOINT=${selected[0]}
SCORE_FIELD=${selected[1]}
TPI_TYPED_RESIDUAL_DECAY_STEPS=${selected[2]}
TPI_TYPED_RELIABLE_MARGINAL_WEIGHT=${selected[3]}
TPI_TYPED_RELIABLE_MIN_HEADS=${selected[4]}
TPI_TYPED_RELIABLE_CP0_MIN_HEADS=${selected[5]}
TPI_ADAPTIVE_BASE_CANDIDATES=${selected[6]}
TPI_ADAPTIVE_EXPANSION_MARGIN=${selected[7]}
TPI_ADAPTIVE_MARGIN_MODE=${selected[8]}
MAX_CANDIDATES=${selected[9]}
CANDIDATE_STRATEGY=${selected[10]}
TPI_LATENT_REENCODE_INTERVAL=${selected[11]}
TPI_LATENT_REENCODE_BLEND=${selected[12]}

if [[ "$SCORE_FIELD" != q_typed_reliable_context ]]; then
  echo "alpha-boundary refinement requires q_typed_reliable_context, got $SCORE_FIELD" >&2
  exit 2
fi
...
export TPI_TYPED_RESIDUAL_DECAY_STEPS TPI_TYPED_RESIDUAL_CLIP=1.0
...
# The coarse Round7 grid ended at alpha=0.20, so close that open boundary
# before accepting the b15-only winner.
alphas=(0.25 0.30 0.40 0.50)
IFS=',' read -r -a gpus <<< "${GPUS_CSV:-0,1,2,4}"
...
run_one() {
  ...
  local out="$SELECTION/alpha_boundary/alpha_${alpha_tag}"
  local log="$SELECTION/logs/alpha_boundary__a${alpha}.log"
  echo "[b15-alpha-boundary] start alpha=$alpha decay=$TPI_TYPED_RESIDUAL_DECAY_STEPS gpu=$gpu checkpoint=$CHECKPOINT"
  ...
  python -u scripts/run_gmean_sweep.py \
    ...
    2>&1 | sed -u "s|^|[b15-alpha-boundary/a$alpha] |" | tee "$log"
}
