...
variants=(moe_experts_within moe_joint_within)
checkpoints=(best_final_horizon epoch_010)
alphas=(0.10 0.20)
IFS=',' read -r -a gpus <<< "${GPUS_CSV:-0,1,2,6,7}"
...
run_one() {
  ...
  local out="$OUT/$variant/$checkpoint_tag/q_typed_reliable_context/alpha_${alpha_tag}_weight_0p75"
  echo "[b15-r8] start variant=$variant checkpoint=$checkpoint_tag alpha=$alpha gpu=$gpu"
  TPI_TYPED_RESIDUAL_ALPHA="$alpha" TPI_TYPED_RESIDUAL_DECAY_STEPS=16 \
  TPI_TYPED_RELIABLE_MARGINAL_WEIGHT=0.75 CUDA_VISIBLE_DEVICES="$gpu" \
    python -u scripts/run_gmean_sweep.py \
      ...
      2>&1 | sed -u "s|^|[b15-r8/$variant/$checkpoint_tag/a$alpha] |" \
...
python scripts/select_onpolicy_typed_on_b15.py --base "$BASE" \
  --incumbent-manifest autoresearch/loop-260720-0945/model_training_round7/b15_selection/winner.json
