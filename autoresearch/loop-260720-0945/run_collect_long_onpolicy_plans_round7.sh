...
run_plan() {
  ...
  if [[ -s "$out" ]] && (( $(wc -l < "$out") >= 129 )); then
    echo "[onpolicy-r7] skip complete benchmark=$benchmark out=$out"
    return 0
  fi
  echo "[onpolicy-r7] start benchmark=$benchmark gpu=$gpu"
  CUDA_VISIBLE_DEVICES="$gpu" python -u -m tpi_jepa.plan \
    --checkpoint "$CHECKPOINT" --benchmark-id "$benchmark" --budget 128 \
    ...
    2>&1 | sed -u "s|^|[onpolicy-r7/$benchmark] |" | tee "$PLANS/logs/$benchmark.log"
  echo "[onpolicy-r7] done benchmark=$benchmark gpu=$gpu"
}
...
echo "[onpolicy-r7] plans_complete count=${#benchmarks[@]} out=$PLANS"
