...
# All settings are global and selected on b15 only.  Small residual weights
# preserve the frozen Round4 context policy while testing the cone head.
settings=(
  "q_typed_residual_context 0.02 16"
  "q_typed_residual_context 0.05 16"
  "q_typed_residual_context 0.10 16"
  "q_typed_residual_context 0.20 16"
  "q_typed_reliable_context 0.05 16"
  "q_typed_reliable_context 0.10 16"
  "q_typed_trust_context 0.05 16"
  "q_typed_trust_context 0.10 16"
)
IFS=',' read -r -a gpus <<< "${GPUS_CSV:-1,2,4,5,6}"
...
run_one() {
  ...
  local out="$OUT/$variant/$checkpoint_tag/$score/alpha_${alpha_tag}_decay_$decay"
  local checkpoint="$BASE/runs/$variant/$checkpoint_tag.pt"
  echo "[b15-round6] start variant=$variant checkpoint=$checkpoint_tag score=$score alpha=$alpha decay=$decay gpu=$gpu"
  ...
  CUDA_VISIBLE_DEVICES="$gpu" \
    python -u scripts/run_gmean_sweep.py \
      ...
      2>&1 | sed -u "s|^|[b15-round6/$variant/$checkpoint_tag/$score/a$alpha/d$decay] |" \
