...
OUT="autoresearch/loop-260720-0945/typed_winner_round2_five"
if [[ ! -f "$WINNER" ]]; then
    echo "missing b15-only selection manifest: $WINNER" >&2
    exit 1
fi
mapfile -t selected < <(python -c \
    'import json,sys; w=json.load(open(sys.argv[1]))["winner"]; print(w["checkpoint"]); print(w["score_field"]); print(w["typed_residual_alpha"])' \
    "$WINNER")
...
export TPI_TYPED_RESIDUAL_ALPHA
...
run_one() {
    ...
    echo "[typed-five-r2] start circuit=$circuit gpu=$gpu checkpoint=$CHECKPOINT score=$SCORE_FIELD alpha=$TPI_TYPED_RESIDUAL_ALPHA"
    CUDA_VISIBLE_DEVICES="$gpu" python -u scripts/run_gmean_sweep.py \
        ...
        | sed -u "s|^|[typed-five-r2/$circuit] |" \
        | tee "$OUT/logs/$circuit.log"
    echo "[typed-five-r2] done circuit=$circuit gpu=$gpu"
}
