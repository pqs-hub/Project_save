...
BASE=$LOOP/model_training_round15_candidate_prior_fine_b15
...
ALPHAS=(0.014 0.018 0.022 0.026)
...
for index in "${!ALPHAS[@]}"; do
  ...
  gpu=${GPUS[$index]}
  tag=$(alpha_tag "$alpha")
  echo "[r15-plan] start alpha=$alpha gpu=$gpu"
  TPI_CANDIDATE_PRIOR_ALPHA="$alpha" CUDA_VISIBLE_DEVICES="$gpu" \
    ...
    python -u -m tpi_jepa.plan \
      ...
      2>&1 | sed -u "s|^|[r15-plan/$alpha] |" | tee "$BASE/logs/plan_${tag}.log" &
  pids+=("$!")
done
wait_all "${pids[@]}"
...
for alpha in "${ALPHAS[@]}"; do
  tag=$(alpha_tag "$alpha")
  echo "[r15-eval] start alpha=$alpha"
  /usr/bin/time -f '%e' -o "$BASE/timing/eval_${tag}_sec.txt" \
    python -u -m tpi_jepa.evaluate_plan_tmax \
      ...
      2>&1 | sed -u "s|^|[r15-eval/$alpha] |" | tee "$BASE/logs/eval_${tag}.log" &
  pids+=("$!")
done
wait_all "${pids[@]}"
