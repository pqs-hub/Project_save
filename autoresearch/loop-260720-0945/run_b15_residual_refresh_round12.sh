...
BASE=$LOOP/model_training_round12_restored
...
mkdir -p "$BASE/logs" "$BASE/prefix_evals" "$BASE/residual_priors" "$BASE/plans" "$BASE/evals"
if (( ${#GPUS[@]} < ${#STEPS[@]} )); then
...
for step in "${STEPS[@]}"; do
  out="$BASE/prefix_evals/step_$(printf '%03d' "$step")"
  echo "[r12-prefix] start step=$step"
  python -u -m tpi_jepa.evaluate_plan_tmax \
    --benchmark-id iscas99__b15_1 --plan-csv "$PREFIX_PLAN" --out-dir "$out" \
    --backend atalanta-bist --patterns 300000 --seed 2026 --eval-step-mode final \
    --max-steps "$step" \
    2>&1 | sed -u "s|^|[r12-prefix/$step] |" | tee "$BASE/logs/prefix_${step}.log" &
  pids+=("$!")
done
wait_all "${pids[@]}"
...
for step in "${STEPS[@]}"; do
  ...
  mkdir -p "$BASE/residual_priors/$tag"
  python -u scripts/build_real_fault_priors.py "$BASE/prefix_evals/$tag/labels.csv" \
    --last-row-only \
...
for index in "${!STEPS[@]}"; do
  ...
  gpu=${GPUS[$index]}
  tag="step_$(printf '%03d' "$step")"
  echo "[r12-plan] start step=$step gpu=$gpu"
  CUDA_VISIBLE_DEVICES="$gpu" python -u -m tpi_jepa.plan \
    ...
    --device cuda --out "$BASE/plans/$tag.csv" \
    2>&1 | sed -u "s|^|[r12-plan/$step] |" | tee "$BASE/logs/plan_${step}.log" &
  pids+=("$!")
done
wait_all "${pids[@]}"
...
for step in "${STEPS[@]}"; do
  ...
  python -u -m tpi_jepa.evaluate_plan_tmax \
    ...
    2>&1 | sed -u "s|^|[r12-eval/$step] |" | tee "$BASE/logs/eval_${step}.log" &
  pids+=("$!")
done
wait_all "${pids[@]}"
