...
BASE=$LOOP/model_training_round17_typed_residual_fine_b15
...
PRIOR=$LOOP/model_training_round16_typed_residual_b15/residual_priors/real_fault_priors.csv
...
STRENGTHS=(0.60 0.65 0.70 0.80 0.85 0.90)
...
for index in "${!STRENGTHS[@]}"; do
  ...
  echo "[r17-plan] start polarity=$strength gpu=$gpu"
  TPI_HARD_CLUSTER_FAULT_POLARITY_ALPHA="$strength" CUDA_VISIBLE_DEVICES="$gpu" \
    ...
    python -u -m tpi_jepa.plan \
      ...
      2>&1 | sed -u "s|^|[r17-plan/$strength] |" | tee "$BASE/logs/plan_${tag}.log" &
  pids+=("$!")
done
wait_all "${pids[@]}"
...
for strength in "${STRENGTHS[@]}"; do
  tag=$(strength_tag "$strength")
  echo "[r17-eval] start polarity=$strength"
  /usr/bin/time -f '%e' -o "$BASE/timing/eval_${tag}_sec.txt" \
    python -u -m tpi_jepa.evaluate_plan_tmax \
      ...
      2>&1 | sed -u "s|^|[r17-eval/$strength] |" | tee "$BASE/logs/eval_${tag}.log" &
  pids+=("$!")
done
wait_all "${pids[@]}"
