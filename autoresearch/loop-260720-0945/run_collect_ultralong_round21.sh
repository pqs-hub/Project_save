...
ROOT=autoresearch/loop-260720-0945/model_training_round21
PLANS=$ROOT/ultralong_onpolicy_plans
MANIFEST=$ROOT/ultralong_source_manifest.json
OUT=$ROOT/ultralong_prefix_oracle
...
python -u scripts/prepare_ultralong_round21.py | tee "$ROOT/prepare_ultralong_sources.log"
...
IFS=',' read -r -a gpus <<< "${GPUS_CSV:-1,4,5,6,7}"
...
run_plan() {
  ...
  local old=autoresearch/loop-260720-0945/model_training_round20/late_onpolicy_plans/$benchmark.csv
  local -a prefix_args=()
  if [[ -s "$out" ]] && (( $(wc -l < "$out") >= 769 )); then
    echo "[onpolicy-r21] skip complete benchmark=$benchmark out=$out"
    return 0
  fi
  if [[ -s "$old" ]] && (( $(wc -l < "$old") >= 257 )); then
    prefix_args=(--prefix-plan "$old" --prefix-steps 256 --prefix-state-mode replay)
  fi
  echo "[onpolicy-r21] start benchmark=$benchmark gpu=$gpu reused_prefix=${#prefix_args[@]}"
  CUDA_VISIBLE_DEVICES="$gpu" python -u -m tpi_jepa.plan \
    --checkpoint "$CHECKPOINT" --benchmark-id "$benchmark" --budget 768 \
    ...
    "${prefix_args[@]}" \
    2>&1 | sed -u "s|^|[onpolicy-r21/$benchmark] |" | tee "$PLANS/logs/$benchmark.log"
  echo "[onpolicy-r21] done benchmark=$benchmark gpu=$gpu"
}
...
echo "[onpolicy-r21] plans_complete count=${#benchmarks[@]} out=$PLANS"
...
python -u scripts/collect_onpolicy_prefix_oracle.py \
  ...
  --prefix-steps 320,448,576,704,767 \
