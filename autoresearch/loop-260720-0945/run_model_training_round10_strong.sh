...
run_one() {
  ...
  echo "[train-r10-strong] start variant=$variant gpu=$gpu"
  CUDA_VISIBLE_DEVICES="$gpu" python -u -m tpi_jepa.train --config "$BASE/configs/$variant.json" \
    2>&1 | sed -u "s|^|[train-r10-strong/$variant] |" | tee "$BASE/logs/$variant.log"
}
...
echo "[train-r10-strong] complete variants=${variants[*]}"
