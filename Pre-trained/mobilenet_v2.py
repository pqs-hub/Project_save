...
# 2. 使用预训练的 MobileNetV2 模型，去除最后的全连接层
...
# 9. 保存聚类结果
def save_clustered_images(image_paths, labels, output_dir, model_name):
    # 创建主目录
    ...
    for cluster in range(n_clusters):
        # 创建簇索引目录
        cluster_dir = os.path.join(model_output_dir, f'Cluster_{cluster}')
...
model_name = "mobilenet_v2"  # 指定模型名称
save_clustered_images(image_paths, kmeans.labels_, output_directory, model_name)
