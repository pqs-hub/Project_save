"""Collect per-variant Q-v2 train/eval summaries into one table."""
...
def main() -> None:
    ...
    parser.add_argument("--out-dir", type=Path, default=Path("autoresearch/q-v2-ablation-260630"))
    ...
    write_tsv(args.out_dir / "q_v2_ablation_summary.tsv", rows)
    write_report(args.out_dir / "q_v2_ablation_report.md", rows)
    print(f"wrote {args.out_dir / 'q_v2_ablation_summary.tsv'}")
    print(f"wrote {args.out_dir / 'q_v2_ablation_report.md'}")
