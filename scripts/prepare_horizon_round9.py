#!/usr/bin/env python3
"""Prepare Round9 horizon-conditioned MoE training configs."""
...
LATE_ORACLE = ROOT / "late_prefix_oracle/oracle_actions.tsv"


VARIANTS = {
    "horizon_only_late": {
        "trainable_modules": "typed_horizon_only",
        "lr": 1.0e-4,
...
def main() -> None:
    ...
    old_manifest = validate_prefix_oracle(
        LONG_ORACLE,
    ...
    print(
        "validated horizon oracle "
        f"old_states={old_manifest['state_count']} "
    ...
    provenance = {
        "round": 9,
        "purpose": "explicit horizon conditioning trained on late real ATPG marginals",
        "selection_circuit": "b15_C only",
        "validation_circuits": ["b15_C", "b20_C", "b21_C", "b22_C", "b17_C"],
        "init_checkpoint": str(INIT),
        "target_circuits_in_training": False,
        "old_oracle_states": old_manifest["state_count"],
